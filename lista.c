...
{
	int i, v = 0, vetVertices[numNohs], totalPassado = 0;
	...
	if (l)//Verifica se é NULL o valor da lista
	{
		auxPos = aux = l;
		...
		if (!aux->adj)
		{
			printf("\n\nO vertice inicial nao possui nenhum adjacente, \no grafo nao eh conexo.\n");
			return 0;
		}
		else
			while (aux)
			{
				...
				//Testa para verificar se todos os nós foram alcançados
				if (totalPassado == numNohs)
					return 1;
			}
	}
